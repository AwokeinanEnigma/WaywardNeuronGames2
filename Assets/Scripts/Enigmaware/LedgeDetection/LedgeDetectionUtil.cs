...
namespace Enigmaware.Core
{
    public static class LedgeDetectionUtil
    {
        ...
        public static bool TryFindLedge(Ray ray, LedgeDetectionSettings settings, out Ledge ledge,
            out LedgeDetectionResults result, bool drawGizmos = false)
        {
                    ...
                    settings.GroundLayers))
            {
                ...
                for (int i = 0; i < settings.MaxSurfaceRaycastSteps; i++)
                {
                    ...
                    if (Physics.CheckSphere(overlapPoint, .1f) == false)
                    {
                        ...
                        var surfaceRay = new Ray(overlapPoint + Vector3.up + -wallHit.normal.normalized * .5f,
                            Vector3.down);

                        if (Physics.Raycast(
                                surfaceRay,
                                out var surfaceHit,
                                settings.MaxSurfaceRaycastStepInterval * 4f,
                                settings.GroundLayers,
                                QueryTriggerInteraction.Ignore))
                        {
                            ...
                            var boxCenter = surfaceHit.point + // start at the point we found on the surface
                                            Vector3.up * settings.ClearanceHeight *
                                            .5f + // move up by half clearance height
                                            Vector3.up * .02f; // and some extra so we dont intersect floor

                            var boxSize = new Vector3(settings.ObstructionCheckSize, settings.ClearanceHeight,
                                settings.ObstructionCheckSize);
                ...
                if (foundSurface)
                {
                            ...
                            settings.GroundLayers))
                    {
                        ...
                        if (drawGizmos)
                        {
                            GizmosEx.DrawArrow(groundRay.origin, groundRay.direction * groundHit.distance,
                                Color.yellow);
                        }
#endif
                    }
                    ...
                    if (drawGizmos)
                    {
                        ...
                        GizmosEx.DrawArrow(surfacePoint + Vector3.up * settings.ClearanceHeight,
                            Vector3.down * settings.ClearanceHeight, Color.green);
                    }
#endif
